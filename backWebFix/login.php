<?php
	session_start();
	include "connect.php";
	if(	isset($_POST["login-submit"]) && !empty($_POST["user"]) && !empty($_POST["password"]) )
	{
		$grabUser = $_POST["user"];
		$grabPassword = $_POST["password"];
		
		$sql = mysqli_query($link,"SELECT * FROM tbladmin WHERE nama = '$grabUser' AND password = '$grabPassword' ");
		if($row = mysqli_num_rows($sql))
		{
			$_SESSION["admin"]=$grabUser;
